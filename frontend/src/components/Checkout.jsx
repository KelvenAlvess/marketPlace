  ...
  const handleCardSubmit = async (cardFormData) => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (!methodId) {
          throw new Error("Não foi possível identificar a bandeira do cartão. Tente novamente.");
        }
        ...
        if (!msg) msg = err.message || 'Erro ao processar pagamento com cartão';
        if (msg.includes("same")) {
          msg = "⚠️ ERRO SANDBOX: Use um e-mail diferente do vendedor.";
        }
        setError(msg);
        reject();
      } finally {
        ...
        <div className="flex flex-col lg:flex-row gap-8 min-h-[500px]">
          ...
          <div className="flex-1">
            <div className="flex items-center mb-8">
              ...
              <span className="ml-2 mr-4">Dados</span>
                ...
                <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
                  ...
                  <form onSubmit={handleAddressSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <button type="submit" disabled={loading} className="md:col-span-2 bg-gray-900 text-white py-3 rounded font-bold hover:bg-black transition-colors mt-4">
                      {loading ? 'Salvando...' : 'Ir para Pagamento'}
                    </button>
                  </form>
                </div>
            )}
            ...
            {step === 2 && (
                <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
                  <div className="flex justify-between mb-6">
                    ...
                    <button onClick={() => setStep(1)} className="text-sm text-blue-600 underline">Editar Dados</button>
                  </div>
                  ...
                  {paymentType === 'card' && (
                      <div key="card-form">
                        <CardPayment
                            initialization={{ amount: orderData.totalAmount }}
                  ...
                  {paymentType === 'pix' && (
                      <div className="text-center">
                        <button onClick={handlePix} disabled={loading} className="bg-green-600 text-white w-full py-3 rounded font-bold hover:bg-green-700">
          ...
          {/* DIREITA: RESUMO DO PEDIDO (ATUALIZADO PARA CARDS) */}
            ...
            <div className="bg-gray-50 p-6 rounded-xl sticky top-24 border border-gray-200 shadow-sm">
              ...
              <div className="space-y-4 mb-6 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                {orderData.items?.map((item, i) => {
                  // Tenta pegar a imagem do produto. Se não tiver, usa fallback
                  const itemImage = item.product?.image || item.image || 'https://images.unsplash.com/photo-1511707171634-5f897ff02aa9?auto=format&fit=crop&w=150&q=80';
                  ...
                  return (
                      <div key={item.orderItemId || i} className="flex gap-3 bg-white p-3 rounded-lg border border-gray-100 shadow-sm">
                        ...
                        <div className="flex flex-col justify-center flex-1 min-w-0">
                          <h4 className="text-sm font-bold text-gray-800 line-clamp-1" title={item.productName}>
                            {item.productName}
                          </h4>
                          <div className="flex justify-between items-end mt-1">
                            <div className="text-xs text-gray-500">
                              <span className="font-semibold text-gray-700">{item.quantity}x</span> R$ {unitPrice.toFixed(2)}
                            </div>
              ...
              <div className="border-t border-gray-200 pt-4 space-y-2">
                ...
                <div className="flex justify-between items-center pt-4 mt-2 border-t border-dashed border-gray-200">
                  ...
                  <span className="text-2xl font-extrabold text-red-600">R$ {orderData.totalAmount?.toFixed(2)}</span>
                </div>
              </div>
            </div>
          </div>
